...
export default function OrderDetailPage() {
    ...
    const id = useMemo(() => decodeURIComponent(String(params?.id ?? "")), [params]);
    ...
    const fetchData = async () => {
        try {
            ...
            setError(e?.response?.data?.message || e?.message || "Gagal mengambil data order");
        } finally {
            setLoading(false);
        }
    };
    ...
    const updateOrder = async (payload: any) => {
        try {
            ...
            alert(e?.response?.data?.message || e?.message || "Terjadi kesalahan saat update");
        } finally {
            setUpdating(false);
        }
    }
                        ...
                        <CardContent className="p-4 space-y-3">
                            <div className="flex items-start justify-between gap-3">
                                ...
                                <div className="flex flex-col gap-2">
                                    <div className="text-xs text-gray-500 text-right">Update Order Status</div>
                                    <select
                                    ...
                                    {updating && <span className="text-[10px] text-orange-500 text-right animate-pulse">Updating...</span>}
                                </div>
                            </div>
                                    ...
                                    {products.map((p: any) => (
                                        ...
                                        >
                                            <div className="flex justify-between items-start gap-2">
                                                ...
                                                {Array.isArray(p?.variants) && p.variants[0]?.quantity != null && (
                                            ...
                                            <div className="flex flex-col gap-1.5 mt-auto pt-2 border-t border-gray-100">
                                                <div className="text-[10px] text-gray-400 uppercase tracking-wider font-bold">Status Produk</div>
                                                <select
